...
import { Mail, Send, Inbox, Send as EnviadosIcon, Trash2, Check, Plus, X } from 'lucide-react';
...
export default function AdminMensajesPage() {
  ...
  const [activeTab, setActiveTab] = useState<'recibidos' | 'enviados' | 'nuevo'>('recibidos');
  ...
  const fetchMensajes = async (tipo: 'recibidos' | 'enviados') => {
    setLoading(true);
    try {
  ...
  const fetchStaff = async () => {
    try {
      const res = await fetch('/api/admin/mensajes/staff', { headers });
      const json = await res.json();
      if (json.success) setStaff(json.data as StaffUser[]);
    } catch {
      // ignore
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8">
        <div className="max-w-5xl mx-auto">
          ...
          {activeTab === 'nuevo' && (
            <form onSubmit={handleEnviar} className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
              <div>
